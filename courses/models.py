...
# 5. Група студентів (ТУТ ВАЖЛИВА ПРАВКА ДЛЯ ВЧИТЕЛЯ)
class StudentGroup(models.Model):
    name = models.CharField(max_length=100)
    subject = models.ForeignKey(Subject, on_delete=models.CASCADE)
    # Ми використовуємо related_name='teaching_groups' для зручності Людини 2
...
class LessonTemplate(models.Model):
    group = models.ForeignKey(StudentGroup, on_delete=models.CASCADE, related_name='templates')
    day_of_week = models.IntegerField(choices=[(i, i) for i in range(1, 8)])
    start_time = models.TimeField()

# 7. Конкретний Урок
class Lesson(models.Model):
    group = models.ForeignKey(StudentGroup, on_delete=models.CASCADE, related_name='lessons')
...
class Attendance(models.Model):
    ...
    class Meta:
        # Щоб не було дублікатів відвідуваності
        unique_together = ('student', 'lesson')

# 9. Склад абонемента
class SubscriptionPlanSubject(models.Model):
    plan = models.ForeignKey(SubscriptionPlan, on_delete=models.CASCADE, related_name='plan_subjects')
    subject = models.ForeignKey(Subject, on_delete=models.CASCADE)

    class Meta:
        unique_together = ('plan', 'subject')

# 10. Абонемент студента
